...
- `creative_overlays.headline` は一目で読める一訴求、原則14文字以内にする。`subline` は必要な場合だけ原則20文字以内にする。詳細や注意書きは投稿本文へ残し、画像に小さい文字で詰め込まない。内容や条件を省くと誤解を招く訴求は選ばない。
- 幅360pxのスマホ表示で見出し20〜24px・補足14px相当になる。見出しは2〜3行で読ませ、横長・正方形では指定側の幅60%以内に十分な背景余白を確保する。ストーリーは上下14%を空ける。小さな文字を前提にした余白や構図にはしない。
...
- `sourceSnapshot` の商品ポイント・商品紹介文・仕様から、異なる特徴を十分に拾う。Instagram・ストーリーは4〜6か所（標準5か所）、横長のX・Threadsは3〜4か所を目安にする。原材料、食感、味、製法、内容量・セット内容などから商品を理解できる注釈を選び、情報があるのに2か所だけで終えない。根拠が不足する場合は同じ特徴の言い換えで水増しせず数を減らす。見本の重量、具材や文言を流用せず、入力で確認できる値・単位だけを記載する。盛付例とセット内容を混同しない。
...
- 注釈は1か所につき短い見出しと必要なら補足の最大2行、合計12〜18文字を目安にする。素材名・数値だけを置かず、何に使われているか、何が特徴かを単独で理解できる文言にする。幅360px表示で文字高20〜24px相当を目標に、商品周囲の上・左右・下へ分散し、余白を使って特徴を囲む構図にする。矢印同士・文字・商品の主要部分を重ねない。文字を縮める前に短い文言と配置を調整する。左右8%、ストーリー上下14%に重要文字を置かない。
