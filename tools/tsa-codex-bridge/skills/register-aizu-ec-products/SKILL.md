...
- QSMの商品照会にはJAN専用検索欄がない。JAN重複確認は画面上の「リスト編集」で「商品識別コード」列を一時表示し、販売ステータスを「全体」、期間を十分過去から現在、表示件数を最大にしてQoo10商品の全ページを読む。対象13桁がどの行にもないことを確認する。JavaScriptソース、非公開API、別ブラウザは使わない。
...
5. 画像はQoo10の外部URLアップロードを使い、`role=main`をメイン画像、残りを`order`順の追加画像にする。URL以外の画像や順序を使わない。保存後にQoo10 CDNのURLへ変換された場合は、保存済みサムネイルの内容を元画像と順番ごとに照合し、7枚すべて一致した時だけ `image_sources_verified=true` とする。CDN URLの文字列が元URLと違うこと自体は失敗にしない。
...
7. 商品照会上部の総件数にMOVE商品が含まれる場合がある。Qoo10通常商品一覧の実行結果とページャーを基準に全ページを確認し、総件数表示だけで未確認ページがあると推測しない。
