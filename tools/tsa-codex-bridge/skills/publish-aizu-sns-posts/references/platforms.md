...
最新AX状態の可視添付ボタンをtab.clickで押し、公式file chooserへ固定画像を渡す。ブラウザーの確認要求はBridgeが表示し、回答後に同じ処理が続く。単なるchooserタイムアウトを権限拒否と混同しない。Threadsで自動リンクカードが添付ボタンを隠す場合は、本文URLを保持して自動カードだけ外す。
