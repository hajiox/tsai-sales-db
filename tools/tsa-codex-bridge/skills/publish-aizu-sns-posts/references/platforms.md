...
- 画像プレビューの描画と処理が終わり、進捗表示が消えて「ポストする」が有効になってから押す。押下後は最大45秒、成功通知・入力欄消去・画面遷移を待ち、最新のstatus/alertとプロフィールで確認する。
- 同じ本文・画像1枚・有効な投稿ボタンが残り、処理中・成功・エラー表示が一度もなく、待機後のプロフィールにも一致投稿がない場合だけ、送信自体が発生しなかったものとして2回目を許可する。合計2回までとし、送信結果が不明なら再押下しない。
