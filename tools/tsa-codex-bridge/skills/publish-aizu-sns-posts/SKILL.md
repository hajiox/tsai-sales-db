...
- X: 固定済み `post_text` と画像を通常投稿する。予約UIは使わない。画像プレビューが完全に表示され、アップロード中・処理中の表示が消え、「ポストする」が有効になってから押す。押下後は最大45秒、成功通知、入力欄の消去、画面遷移のいずれかを待ち、最新のstatus/alertとプロフィールを確認する。同じ本文と画像1枚が残り、ボタンが有効なまま、処理中・成功・エラーの表示が一度もなく、待機後のプロフィールにも一致投稿がない場合だけ、最初の押下が送信されなかった決定的証拠として2回目を許可する。合計2回を超えて押さない。
