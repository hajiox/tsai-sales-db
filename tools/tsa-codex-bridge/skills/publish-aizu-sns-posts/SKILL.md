...
3. 「編集」から「テキスト」へ進み、「テキストを追加」で固定済み `story_text` を入力する。全体プレビューで無理なく読める大きさまで拡大し、原則2〜4行かつ画像幅の半分以上を使う文字ブロックにする。1行の小さな表示、全体プレビューで読めない表示は不合格とし、公開前にサイズを直す。追加直後に選択中のテキストをドラッグして画像内の安全領域へ置き、文字全体が欠けず、商品を不自然に隠していないことをスクリーンショットで確認する。
