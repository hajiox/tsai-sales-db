...
- X: 固定済み `post_text` と画像を通常投稿する。予約UIは使わない。Xの複数行編集欄へ `fill` で本文全体を一括入力しない。`post_text` の改行をLFへ統一して行へ分け、空の編集欄をクリックし、各行を `type_text`、元の改行1つごとに `press_key Enter` 1回の順で入力する。空行は文字を入力せずEnterだけを押す。画像添付前と投稿直前に、編集欄内のDraft.js `data-block="true"` の各行をLFで連結した値が固定本文と完全一致し、特にURLと後続ハッシュタグが別行であることを確認する。不一致なら `Control+A` と `Backspace` で消して同じ行単位入力を1回だけやり直し、再び不一致なら投稿しない。本文が完全一致した後、画像プレビューが完全に表示され、アップロード中・処理中の表示が消え、「ポストする」が有効になってから押す。Xの投稿ボタンを押すたびに、直後に必ず`wait_for`をtimeout 45000で実行し、成功通知または入力欄消去を最大45秒待つ。直後のsnapshotや`evaluate_script`だけで待機を代用しない。同じ固定本文が入力欄に残る間はプロフィールへ移動しない。最初の45秒待機後、新しいsnapshotで同じ本文と画像1枚、有効な新しい投稿ボタン、処理中・成功・エラー表示なしをすべて確認できた場合だけ、その新しいボタンを2回目として押す。再び45秒待ってからstatus/alertとプロフィールを確認する。2回目の判断前にプロフィールへ移動すると下書きが破棄されるため禁止する。合計2回を超えて押さない。
...
事前に `references/platforms.md` の確認済みビジネス・ページ対応を使って対象を選択する。初期ページの「Instagramをリンク」表示やビジネス名とハンドルの相違だけで未連携と判断せず、正しいビジネス/ページで指定Instagramを照合する。
...
3. 「編集」から「テキスト」へ進み、「テキストを追加」で固定済み `story_text` を入力する。全体プレビューで無理なく読める大きさまで拡大し、固定本文を20文字以下の2〜3枠へ分け、各枠1〜2行かつ画像幅の半分以上を使う。連結した本文は固定値と一致させる。1行の小さな表示、全体プレビューで読めない表示は不合格とし、公開前にサイズを直す。追加直後に選択中のテキストをドラッグして画像内の安全領域へ置き、文字全体が欠けず、商品を不自然に隠していないことをスクリーンショットで確認する。
...
7. Meta Business Suiteの作成画面右側プレビューは、適用済みテキストとスタンプを表示しない場合がある。表示欠落だけで編集失敗と断定せず、「編集」を1回だけ開き直し、文字とスタンプが焼き込まれた保存済み画像をスクリーンショットで確認する。textareaの消失は失敗ではない。二重追加せず「キャンセル」で作成画面へ戻る。
8. 公開直前に「シェア先」がInstagramの指定アカウントだけであること、画像が1枚であること、編集画面に固定文言とリンクスタンプが保持され、全文が読みやすい大きさであることを再確認する。Facebookページが選択されたまま、リンクスタンプがない、文言が欠ける、文字が小さく読めない、リンク先を確認できない場合は公開しない。公開成功の `evidence` や `message` に文字が小さい・読めない状態を記録して成功扱いにしない。

## 投稿前後の安全確認

1. 投稿直前に対象アカウント、対象媒体、本文、画像、リンクを再確認する。
2. 投稿ボタンは媒体ごとに最大1回だけ押す。Xだけは上記の決定的な未送信条件をすべて確認できた場合に限り2回目を許可する。公開済みか不明な状態では再押下しない。
