...
- Before downloading, read the page's maximum available month (`#maxMonth[data-max-month]`). If the requested `YYYYMM` is newer, return `needs_review` with the requested month and current maximum. Do not substitute settlement statements whose close dates straddle months. TSA keeps a provisional estimate based on prior official monthly ratios, and the Bridge retries incomplete months daily so the official import replaces that estimate when published.
...
- If any included non-ad cost row remains `概算`, set `coverage_level: partial`, list the affected items in `notes`, and let the daily retry replace it after BillPay changes them to `確定`. Use `complete` only when the requested month is available and all included rows are confirmed.
...
- In QSM open `精算管理 > 販売内訳` (`/GMKT.INC.Gsm.Web/Account/SellingReport.aspx`). Check both the upper `販売精算内訳` summary and the lower `販売詳細内訳`.
- For a TSA calendar sales month, set both searches to `購入者の決済日` and the exact inclusive period. The upper search and lower search have separate date controls and separate `検索` buttons; changing only the upper search is insufficient.
- Export the lower `販売詳細内訳` with `Excel` when rows exist. Confirm order number, purchaser payment date, settlement amount/status, sales price, seller-funded discount, Qoo10-funded discount, Qoo10 service fee, shipping, refunds, and adjustments.
- If both settlement searches return zero, cross-check a period-matched official QSM order/delivery export before concluding that the month has no sales. Required evidence columns include `注文日`, `入金日`, `数量`, `購入者決済金額`, `販売価格`, `割引額`, and `注文金額の合計`.
- When the order/delivery export contains transactions but the settlement screens contain no rows, preserve the official order/delivery export and settlement evidence, return `needs_review`, and identify the missing settlement detail. Do not create zero settlement data and do not return `waiting_for_user` unless a real login, MFA, CAPTCHA, account-selection, or download-permission action is required. TSA keeps the official food-category fee estimate until a later retry obtains settlement rows.
- A saved zero-row or unavailable screenshot expires after that run. Every retry must search both current settlement sections again; only stable period-matched order or delivery exports may be reused without repeating their download.
