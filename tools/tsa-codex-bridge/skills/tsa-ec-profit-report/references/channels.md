...
- In QSM open `精算管理 > 販売内訳` (`/GMKT.INC.Gsm.Web/Account/SellingReport.aspx`). Search the upper `販売精算内訳` and lower `販売詳細内訳` separately. Use `精算日` for settlement-cycle reconciliation; use `購入者の決済日` only as a secondary order-period cross-check. A zero result for purchaser payment date never proves that settlement is unpublished.
...
- Also open `Qアカウント > Qアカウント履歴` and search a range covering every computed Wednesday settlement date. Preserve or export each `販売代金 精算` entry. This ledger proves whether the settlement cycle occurred, but an aggregate credit alone does not prove the itemized fee classification.
- If the conservative latest settlement bound has passed and Q account contains the matching settlement credits, never report `official publication waiting`. Reconcile the detail export. If the detail grid/export still cannot be obtained, return `needs_review` with `official settlement completed; detailed reconciliation unavailable`, the expected dates, observed Q account credits, and the exact missing fields.
