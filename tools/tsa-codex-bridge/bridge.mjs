...
async function main() {
  while (!stopping) {
    try {
      ...
      if (config.executionMode === "interactive" && config.carrierAppDir) {
        refreshCodexRuntimeIfDue();
        const adapter = await loadCarrierAdapter(config.carrierAppDir, config.executionMode);
        const pending = await adapter.peekCarrierJob();
        if (pending && codexRuntime.ready) {
          const job = validateCarrierJob(pending);
          currentJobIsLocal = true;
          currentJobId = job.id;
          currentJobCodexUsage = emptyCodexUsage();
          lastError = null;
          startDesktopMonitor(job);
          writeBridgeState();
          const finalState = await adapter.runCarrierJob(job, {runCodex: runCarrierCodex, onState: (state) => updateDesktopMonitor(job.id, carrierMonitorPayload(state))});
          updateDesktopMonitor(job.id, carrierMonitorPayload(finalState));
          continue;
        }
      }
      await heartbeat();
      ...
      lastError = currentJobIsLocal ? "ローカル出荷処理が停止しました。出荷画面で状態を確認してください" : error instanceof Error ? error.message : String(error);
...
async function runCarrierCodex({prompt, workDir, resultPath, schemaPath, addDirs = [], onPid, onBrowserConfirmation}) {
  ...
  try {
    return await waitForCodexExitWithWatchdog(child, {taskKey:CARRIER_TASK_KEY, terminate:terminateChildProcessTree});
  } finally {
    await waitForCarrierChildClose(child, () => updateDesktopMonitor(currentJobId, {
      status: "waiting_for_user", progress: 95,
      currentStep: "Codexプロセスの終了を待っています。別のブラウザジョブは開始しません",
...
function buildEcPricePlanPrompt(parameters) {
  return [
    ...
    "TASK_JSON.productMappings contains product titles previously matched and confirmed by TSA sales imports for this linked product. For each site, search every supplied mapped title before reporting not_found. A verifiedProductIdentifier is the primary identity key; otherwise an exact mapped title plus matching flavor, set count, form, and storage method is strong identity evidence. If recipeSnapshot.ecProductName conflicts with recipeSnapshot.recipeName, JAN-linked evidence, or a supplied mapped title on flavor, form, or set count, treat ecProductName as untrusted and never use it to select the conflicting listing; block that site when no exact identity remains. recipeSnapshot.fillingQuantity is a manufacturing fill target and can differ from the seller's declared net-content weight. Do not block solely for that numeric difference when the primary identifier or the exact mapped-title evidence proves the same sale unit. Block when the difference changes flavor, form, set count, storage method, or leaves more than one plausible product. Record the discrepancy in unit_evidence. For BASE exclude TikTok-linked titles; for TikTok use the supplied BASE-managed TikTok title. Never substitute a similar title.",
...
function listRecipeSnsGeneratedImages(generatedThreadRoot, expectedCount) {
  if (!generatedThreadRoot || !existsSync(generatedThreadRoot) || !statSync(generatedThreadRoot).isDirectory()) {
    throw new Error("ImageGenの生成フォルダが見つかりません");
  }
  const images = readdirSync(generatedThreadRoot, { withFileTypes: true })
    .filter((entry) => entry.isFile() && new Set([".png", ".jpg", ".jpeg", ".webp"]).has(extname(entry.name).toLowerCase()))
    .map((entry) => {
      const filePath = resolve(generatedThreadRoot, entry.name);
      const verifiedPath = resolveRecipeSnsGeneratedImage(filePath, generatedThreadRoot, generatedThreadRoot);
      const fileStat = statSync(verifiedPath);
      return { filePath: verifiedPath, mtimeMs: fileStat.mtimeMs, name: entry.name };
    })
    .sort((left, right) => left.mtimeMs - right.mtimeMs || left.name.localeCompare(right.name));
  if (images.length !== expectedCount) {
    throw new Error(`ImageGenの生成画像数が一致しません（期待${expectedCount}枚・実際${images.length}枚）`);
  }
...
async function executeRecipeSnsGenerateJob(job) {
  ...
  const prompt = [
    ...
    "Do not browse the web, control a browser, post externally, or modify external data. The only permitted command is reading the built-in imagegen SKILL.md. Do not run Get-ChildItem, Copy-Item, or any command to locate or copy generated images; the Bridge resolves fresh-session ImageGen files deterministically after this run.",
  ...
  for (const platformId of requestedPlatformIds) {
    ...
    const inputPath = parameters.imageMode === "normal" ? sourceImagePath : generatedImagePaths[platformIndex];
    const suppliedGeneratedPath = String(generated.file_path || "").trim();
    if (parameters.imageMode !== "normal" && suppliedGeneratedPath) {
      const verifiedSuppliedPath = resolveRecipeSnsGeneratedImage(suppliedGeneratedPath, workDir, generatedThreadRoot);
      if (verifiedSuppliedPath !== inputPath) {
        throw new Error(`${platform.label}のImageGen生成順と結果パスが一致しません`);
      }
    }
...
function buildRecipeSnsPublishTargetPrompt({ publishSkillText, platformReferenceText, packet }) {
  ...
  return [
    ...
    "Before final submit, verify the visible account, text, image, and link. For X, wait until the image preview is fully rendered, no upload/progress indicator remains, and the visible Post button is enabled before clicking. After the click, wait up to 45 seconds for a success notice, composer clearance, or navigation, then inspect the latest live status/alert and profile result. A second X click is allowed only when the same exact draft and one image remain, the same Post button is still enabled, no busy indicator or success/error notice appeared, and the profile still has no matching post after the wait; those facts clearly prove the first click did not dispatch. Never click more than twice. For other platforms submit at most once unless the UI clearly proves the click did not submit.",
...
function prepareSkillControlledPrompt(taskKey, prompt) {
  const contract = taskKey === CARRIER_TASK_KEY ? CARRIER_SKILL_CONTRACT : TASK_CONTRACTS[String(taskKey || "")];
...
async function heartbeat() {
  ...
  if (lastDesktopTerminalState?.taskKey !== CARRIER_TASK_KEY) reconcileDesktopTerminal(response?.lastTerminal);
...
function workerPayload() {
  ...
  return {
    ...
    version: VERSION,
    currentJobId: currentJobIsLocal ? null : currentJobId,
    lastTerminalJobId: currentJobId || lastDesktopTerminalState?.taskKey === CARRIER_TASK_KEY ? null : lastDesktopTerminalState?.jobId || null,
    lastError: currentJobIsLocal ? null : lastError,
    capabilities: {
      codex: true,
...
async function updateJob(jobId, payload) {
  ...
  if (currentJobIsLocal) return {ok:true};
...
function loadConfig() {
  ...
  const value = {
    ...
    carrierAppDir: String(stored.carrierAppDir || "").trim(),
    reasoningEffort: String(process.env.TSA_CODEX_REASONING_EFFORT || stored.reasoningEffort || "medium").trim().toLowerCase(),
  ...
  if (value.carrierAppDir && !isAbsolute(value.carrierAppDir)) throw new Error("carrierAppDirは絶対パスで指定してください");
...
function bridgeTaskLabel(taskKey) {
  return {
    carrier_monthly_import: "ヤマト・佐川出荷CSV",
