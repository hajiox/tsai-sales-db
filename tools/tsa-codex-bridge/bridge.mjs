...
- Select the Chrome binding exactly once with getForUrl(targetUrl). If an existing matching seller tab is owned by another browser-operation session, leave that operator tab untouched and open at most one temporary tab through the same binding; verify that the temporary tab inherits the expected signed-in account before continuing, then close only that temporary tab. If the selected binding itself detaches or the temporary tab cannot be used, stop as waiting_for_user with a Chrome-control conflict. Never call getForUrl/getDefault/get again to switch to another Chrome or extension instance during this job.
