...
*   **Gemini 2.0 Flash連携**: 画像PDFから直接商品名と単価を抽出し、DBとのファジーマッチング（名称のゆらぎ吸収）を実現。
...
### 2026-02-17 16:04:42 レシピ同期とデータベース修正
- 水のデータベース登録（文字化け修正）および栄養素0設定を完了
- 全369レシピの重量原価再計算を同期


### 2026-02-17 16:10:22 最終デプロイトリガー
- 重複していた「水」データの統合とレシピIDの付け替えを完了
- 本番環境への最終同期を目的としたデプロイ実行


### 2026-02-17 17:02:11 レシピデータ完全同期の完了
- Excel全ファイル（自社OEMネット）から「水」の分量を自動抽出し、全レシピへ紐付け完了
- DB上の「水」を単一項目に統合（栄養素0設定）
- システム全体の合計重量原価を最新データで一括再計算
- 全てのデータ不整合を解消し、本番環境へ反映。
