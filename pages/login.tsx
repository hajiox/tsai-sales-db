import { supabase } from "../lib/supabase";

export default function Login() {
  const handleSignIn = () =>
    supabase.auth.signInWithOAuth({ provider: "google" });

  return (
    <div className="min-h-screen bg-gray-100 flex items-start justify-center pt-36">
      <div className="bg-white p-10 rounded shadow-md text-center space-y-4 w-[320px]">
        <h1 className="text-2xl font-bold text-gray-800">TSA 売上報告システム</h1>
        <p className="text-sm text-gray-500">Technical Staff AI System</p>
        <p className="text-gray-600 text-sm">Googleアカウントでログインしてください</p>
        <button
          className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded w-full"
          onClick={handleSignIn}
        >
          Googleでログイン
        </button>
      </div>
    </div>
  );
}
