...
assert.match(studio, /formatModelName\(selectedGeneration\.model\)/);
assert.match(studio, /selectedGeneration\.failureReason/);
assert.match(studio, /画像を生成できませんでした/);
assert.doesNotMatch(studio, />GPT-5\.6 Sol \/ \{selectedGeneration\.reasoningEffort\}</);
...
assert.match(snsRoute, /failureReason:/);
assert.match(snsRoute, /jobDetails/);
