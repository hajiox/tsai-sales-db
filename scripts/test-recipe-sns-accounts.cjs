...
const release = load("lib/web-sales-codex/bridge-version.ts");
assert.equal(bridge.match(/const VERSION = "([^"]+)"/)[1], release.REQUIRED_TSA_CODEX_BRIDGE_VERSION, "server must accept the distributed Bridge version");
