...
const expectedTasks = {
  ...
  ec_product_register: "register-aizu-ec-products",
...
assert.match(argBuilder, /Resolve routes independently/);
assert.match(argBuilder, /appendChromeDevtoolsMcpArgs\(args, config\.codexHome, \{ workspace: workingDirectory/);
...
assert.doesNotMatch(chromeRelay, /startDaemon/);
assert.match(bridge, /return prepareChromeConnection/);
