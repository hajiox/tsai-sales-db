...
const migrationPath = path.join(
  ...
  "supabase",
  "migrations",
  "20260826194500_recipe_sns_targeted_regeneration_v3.sql",
);
...
const verificationSql = `
  ...
  SELECT
    position($$capabilities->>'recipeSnsProtocolVersion' = '3'$$
      in (SELECT definition FROM claim_function)) > 0 AS recipe_sns_v3_required,
    position($$capabilities->>'recipeSnsProtocolVersion' = '2'$$
      in (SELECT definition FROM claim_function)) = 0 AS recipe_sns_v2_removed,
...
async function verify(client) {
  ...
  const failed = Object.entries(result.rows[0])
    ...
    .map(([key]) => key);
  if (failed.length > 0) throw new Error(`Recipe SNS targeted-regeneration migration checks failed: ${failed.join(", ")}`);
