...
assert.match(installer, /\$interactiveRecoveryTrigger = New-ScheduledTaskTrigger[\s\S]*-RepetitionInterval \(New-TimeSpan -Minutes 1\)/);
assert.match(installer, /-Trigger @\(\$interactiveTrigger, \$interactiveRecoveryTrigger\)/);
assert.match(installer, /\$hasInteractiveRecoveryTrigger[\s\S]*MSFT_TaskTimeTrigger[\s\S]*PT1M/);
