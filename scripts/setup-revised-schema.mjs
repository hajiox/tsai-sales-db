// scripts/setup-revised-schema.mjs
// 改訂版スキーマ作成とデータインポート
...
async function main() {
    ...
    try {
      ...
      CREATE TABLE recipes (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        name TEXT NOT NULL,
        category TEXT NOT NULL, -- 'ネット専用', '自社', 'OEM', 'Shopee'
        is_intermediate BOOLEAN DEFAULT false, -- 【P】で始まるもの
      ...
      CREATE TABLE recipe_items (
        ...
        item_type TEXT NOT NULL, -- 'ingredient', 'material', 'intermediate'
    ...
    `);
        ...
        if (ingredientSheet) {
            ...
            // Row 5から開始（Row 4がヘッダー）
            for (let r = 4; r <= range.e.r; r++) {
                const nameCell = ingredientSheet[XLSX.utils.encode_cell({ r, c: 2 })]; // C列=品名
                ...
                const getNum = (col) => {
                    ...
                    return cell && !isNaN(cell.v) ? parseFloat(cell.v) : null;
                };
        ...
        if (materialSheet) {
            ...
            // Row 5から開始
            for (let r = 4; r <= range.e.r; r++) {
                const nameCell = materialSheet[XLSX.utils.encode_cell({ r, c: 2 })]; // C列=品名
                if (!nameCell || !nameCell.v || nameCell.v === "空白") continue;
           ...
           VALUES ($1, $2, $3, $4, $5)`,
                    [
                        ...
                        priceCell && !isNaN(priceCell.v) ? parseFloat(priceCell.v) : null,
        ...
        for (const { file, category } of files) {
            ...
            for (const sheetName of wb.SheetNames) {
                // システムシートをスキップ
                if (["チャーシュー原価計算", "卸商品原価", "農産物加工記録",
                    "資材総合データベース", "食材総合データベース",
                    "Sheet1", "Sheet2", "Sheet7"].includes(sheetName)) {
                    continue;
                }
                ...
                let devDate = null;
                if (dateCell && dateCell.v && !isNaN(dateCell.v)) {
                    try {
                        const serial = parseInt(dateCell.v);
                ...
                for (let c = 8; c <= 12; c++) {
                    ...
                    if (cell && !isNaN(cell.v) && parseFloat(cell.v) > 0) {
                        sellingPrice = parseFloat(cell.v);
                        break;
                    }
                }
