// /app/components/RakutenCsvImportModal.tsx ver.17 (NextUI対応版)
...
import { Upload, FileCheck, AlertTriangle, CheckCircle, Info } from 'lucide-react';
...
const RakutenCsvImportModal: React.FC<RakutenCsvImportModalProps> = ({ onImportSuccess }) => {
  ...
  return (
    <>
      <Button onPress={onOpen} color="primary" startContent={<Upload size={18}/>}>
        楽天CSVインポート
      </Button>
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} onClose={handleModalClose} size="3xl">
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalFooter>
                <Button color="danger" variant="light" onPress={step === 1 ? onClose : () => setStep(1)} disabled={isLoading}>
                  {step === 1 ? '閉じる' : '← 戻る'}
                </Button>
                {step === 2 && (
