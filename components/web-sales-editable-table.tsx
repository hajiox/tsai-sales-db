// /components/web-sales-editable-table.tsx ver.20 (全文)
"use client";
...
export default function WebSalesEditableTable({ 
  ...
  const handleAddSeries = async () => {
    ...
    try {
      const response = await fetch('/api/series-master', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ series_name: newSeriesName.trim() }) });
      if (response.ok) {
        setNewSeriesName("");
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const response = await fetch('/api/products-master', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ product_name: newProduct.product_name.trim(), series_id: parseInt(newProduct.series_id), product_number: parseInt(newProduct.product_number), price: parseInt(newProduct.price) }) });
      if (response.ok) {
  ...
  const handleDeleteProduct = async (productId: string, productName: string) => {
    ...
    try {
      const response = await fetch('/api/products-master', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ product_id: productId }) });
      if (response.ok) {
        loadData();
        alert('商品が削除されました');
      } else if (response.status === 409) {
        const result = await response.json();
        if (confirm(`「${productName}」には販売実績（${result.sales_count}件）があります。\n\n販売データと一緒に削除しますか？`)) {
          // Force delete logic can be added here
        }
      } else {
        const result = await response.json();
        alert('エラー: ' + result.error);
  ...
  const handleDeleteSeries = async (seriesId: number, seriesName: string) => {
    ...
    try {
      const response = await fetch('/api/series-master', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ series_id: seriesId }) });
      if (response.ok) {
        loadSeries();
        loadData();
        alert('シリーズが削除されました');
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border bg-white shadow-sm">
        <div className="p-3 border-b bg-gray-50 flex justify-between items-center">
          ...
          <button onClick={() => setShowProductForm(!showProductForm)} className="px-3 py-1 bg-green-600 text-white rounded text-sm hover:bg-green-700">{showProductForm ? 'フォームを閉じる' : '商品追加'}</button>
        </div>
        <SalesDataTable rows={rows} editingCell={editingCell} editValue={editValue} savingRows={savingRows} isRowChanged={isRowChanged} onSaveRow={saveRow} onDeleteProduct={handleDeleteProduct} onCellClick={handleCellClick} onEditValueChange={setEditValue} onCellSave={handleCellSave} onCellCancel={handleCellCancel} />
