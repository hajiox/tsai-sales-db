// /components/YahooCsvImportModal.tsx ver.6 (修正UI実装版 - 両パターン対応)
'use client';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-7xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          {step === 1 && (
            <>
              <p className="text-gray-600 mb-4">Yahoo!ショッピングの商品売上CSVをアップロードしてください。</p>
              {error && (
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-4 p-4 border-2 border-dashed rounded-lg">
                  <label htmlFor="yahoo-csv-upload" className="cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium py-2 px-4 rounded-md border border-gray-300 transition-colors">ファイルを選択</label>
                  <Input id="yahoo-csv-upload" type="file" accept=".csv" onChange={handleFileChange} className="hidden" />
              ...
              <Card>
                ...
                <CardContent className="grid grid-cols-3 gap-4">
                  <div className="text-center">
                    ...
                    <div className="text-2xl font-bold text-purple-600">{displayQuantity(parseResult.summary?.totalProducts)}件</div>
                  </div>
                  <div className="text-center">
                    <div className="text-sm text-gray-600">総販売数量</div>
                    <div className="text-2xl font-bold text-purple-600">{displayQuantity(parseResult.summary?.totalQuantity)}個</div>
                  </div>
                  <div className="text-center">
              ...
              <Card>
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-600">
                    Yahoo商品名とマスタ商品を紐付けてください。未マッチの商品は空欄のまま保存されません。
                  </p>
                </CardHeader>
                <CardContent>
                    ...
                    {allMappings.map((mapping, index) => (
                      <div key={index} className={`p-4 border rounded-lg ${mapping.productId ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          <div>
                            <label className="text-sm font-medium text-gray-700">Yahoo商品名</label>
                            ...
                            {mapping.productId && (
                              <div className="mt-2 flex items-center gap-2">
                                <Button
                                  ...
                                  disabled={mapping.isLearned || savingMapping === mapping.yahooTitle}
                                  onClick={() => handleLearnMapping(index)}
                                >
                                  {savingMapping === mapping.yahooTitle ? (
                                    <>学習中...</>
                                  ) : mapping.isLearned ? (
                                    <>
