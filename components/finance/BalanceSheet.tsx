// /components/finance/BalanceSheet.tsx ver.5
"use client";
 ...
 React.useEffect(() => {
   ...
   (async () => {
     try {
       ...
       const a = Math.abs(toNum(totals.find((x) => x.side === 'assets')?.total));
       const l = Math.abs(toNum(totals.find((x) => x.side === 'liabilities')?.total));
       const e = Math.abs(toNum(totals.find((x) => x.side === 'equity')?.total));
...
// ver.5 (2025-08-19 JST) - expose both named and default, fix absolute values
export { BalanceSheet };
