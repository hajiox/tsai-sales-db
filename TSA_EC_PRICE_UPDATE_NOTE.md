...
- Bridge 1.8.24では、EC完了後の商品LPだけの再実行でEC側の旧標準価格を要求しない。LPは許可済みGitHub公開元、商品照合、該当価格箇所、目標価格、デプロイ、本番URLの表示を独立検証する。未参照の重複コンポーネントに同商品の旧価格が残る場合も対象箇所として整合させる。
- Bridge 1.8.25では、LP計画の相対`source_file`を検証済みFresh Cloneのプロジェクトルート基準で解決する。絶対・相対のどちらも同ルート外は拒否し、正しい相対パスを誤って未完了扱いしない。
- Bridge 1.8.26では、検証済みFresh Clone内の対象価格がすべて目標値と一致し、登録済み商品LPの公開URLでも全目標価格を確認できた場合、変更ファイル0件でも冪等な反映済みとして完了する。旧価格が一か所でも残る場合は従来どおり書込・デプロイを必須とする。
