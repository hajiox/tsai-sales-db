...
- Corrected the August Qoo10 product-sales source after EC速報 and the official Qoo10 Shipping API proved 5 orders, 5 items, and JPY 13,330 while the QSM four-screen rule had imported zero.
...
## 2026-09-07 分析画面の最新版追従
- 分析履歴を再取得しても古い選択IDを維持する問題を修正。対象期間の新版を検知したら選択欄・本文を最新版へ切替え、最新版ラベルを表示する。同じ履歴の再取得では手動の過去版選択を維持。
- 実行中3.5秒・待機中15秒の更新と画面復帰時更新を追加。古い通信応答と対象月切替後の応答を除外する。版切替・履歴閲覧・期間切替・削除済み版の回帰テスト、security/RLSを確認。追加の分析ジョブは実行しない。
