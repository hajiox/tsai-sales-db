-- Recipe SNS image/post generation history and guarded Bridge routing.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_web_sales_codex_jobs_one_active_recipe_sns_generation
  ON public.web_sales_codex_jobs ((parameters->>'recipeId'))
  WHERE task_key = 'recipe_sns_generate'
    AND status IN ('queued', 'running');

CREATE TABLE IF NOT EXISTS public.recipe_sns_generations (
  ...
  source_image_id uuid REFERENCES public.recipe_web_images(id) ON DELETE SET NULL,
  source_image_url text NOT NULL,
  source_image_role text NOT NULL CHECK (source_image_role IN ('portrait', 'gallery')),
...
CREATE INDEX IF NOT EXISTS idx_recipe_sns_generations_recipe_created
  ON public.recipe_sns_generations(recipe_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_recipe_sns_generations_status_created
  ON public.recipe_sns_generations(status, created_at DESC);

ALTER TABLE public.recipe_sns_generations ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.recipe_sns_generations FROM anon, authenticated;

-- Route recipe SNS jobs only to a Bridge that advertises protocol v1 and GPT-5.6 Sol.
