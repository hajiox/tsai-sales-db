...
export default function ManualClient() {
  ...
  async function remove(row: Row) {
    const res = await fetch('/api/kpi/manual', {
      ...
      body: JSON.stringify({ metric: row.metric, channel_code: row.channel_code, month: row.month })
    });
        ...
        <form onSubmit={async (e)=>{ e.preventDefault(); await upsert(e.currentTarget); }}>
          <div style={{ display: 'grid', gridTemplateColumns: '160px 160px 1fr 1fr auto', gap: 8, alignItems: 'end' }}>
            <div>
              ...
              <input type="month" name="month" required defaultValue={`${year}-${String(new Date().getMonth()+1).padStart(2,'0')}`} style={{ width: '100%' }} />
            </div>
            <div>
            ...
            <div><button type="submit">保存（UPSERT）</button></div>
          </div>
        </form>
      </section>
      ...
      <section style={{ border: '1px solid #ddd', borderRadius: 8, padding: 16 }}>
        ...
        {loading ? <div>読み込み中…</div> : error ? <div style={{color:'crimson'}}>エラー: {error}</div> : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><tr><th align="left">月</th><th align="left">チャネル</th><th align="right">金額</th><th align="left">メモ</th><th></th></tr></thead>
            <tbody>
              {rows.length === 0 ? (
