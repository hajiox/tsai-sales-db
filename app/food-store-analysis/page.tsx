// /app/food-store-analysis/page.tsx ver.1
"use client"
...
import { TrendingUp, Package, Settings, FolderOpen, Link } from "lucide-react"
import { formatCurrency } from "@/lib/utils"
import { LineChart, Line, BarChart, Bar, ComposedChart, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts'

type SalesData = any;
type ProductMaster = { jan_code: number; category_id: string; product_name: string; };
type CategoryMaster = { category_id: string; category_name: string; };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from('food_store_sales').select('*').eq('report_month', reportMonth).order('total_sales', { ascending: false }),
        supabase.from('food_product_master').select('jan_code, category_id, product_name'),
        supabase.from('food_category_master').select('category_id, category_name').order('display_order'),
      ...
      if (salesData && salesData.length > 0) {
        const productToCategoryMap = new Map(productMaster?.map((p: ProductMaster) => [p.jan_code, { category_id: p.category_id, product_name: p.product_name }]) || []);
        ...
        const enrichedSalesData = salesData.map((sale: SalesData) => {
          const productInfo = productToCategoryMap.get(sale.jan_code);
          const categoryName = productInfo?.category_id ? categoryNameMap.get(productInfo.category_id) : null;
          return {
            ...sale,
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => setShowCategoryModal(true)}>
            <FolderOpen className="h-4 w-4 mr-2" />カテゴリー管理
          </Button>
