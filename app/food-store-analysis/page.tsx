...
function FoodStoreAnalysisContent() {
 ...
 const fetchData = async () => {
   ...
   try {
     ...
     // 売上データ取得（category_idも含む）
     const { data: salesData, error: salesError } = await supabase
       .from('food_store_sales')
       .select('*, food_category_master!food_store_sales_category_id_fkey(category_name)')
     ...
     if (salesData && salesData.length > 0) {
       // category_idがない場合は商品マスターから取得
       const janCodes = salesData.filter(item => !item.category_id).map(item => item.jan_code)
       ...
       if (janCodes.length > 0) {
         ...
         productMap = new Map(productData?.map(p => [p.jan_code, p]) || [])
       }
       ...
       enrichedSalesData = salesData.map(sale => {
         ...
         // 売上データに直接カテゴリーがある場合
         if (sale.food_category_master?.category_name) {
           categoryName = sale.food_category_master.category_name
         }
         // 商品マスターから取得
         else if (!sale.category_id) {
           const product = productMap.get(sale.jan_code)
           categoryName = product?.food_category_master?.category_name || '未分類'
         }
     ...
     setData({
       ...
       hasData: enrichedSalesData.length > 0
     })
   } catch (error) {
   ...
   <div className="space-y-4 p-3 lg:space-y-6 lg:p-6">
     <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
       ...
       <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:flex">
         <Button className="w-full" variant="outline" size="sm" onClick={() => setShowMappingModal(true)}>
           <Link className="h-4 w-4 mr-2" />商品カテゴリー紐付け
         </Button>
         <Button className="w-full" variant="outline" size="sm" onClick={() => setShowCategoryModal(true)}>
