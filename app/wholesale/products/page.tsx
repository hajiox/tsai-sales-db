...
import { ChevronDown, ChevronUp, GripVertical, Plus, Trash2, Search, Link2 } from 'lucide-react';
...
export default function ProductsPage() {
  ...
  const showRecipeLink = (productName: string, link: RecipeLink) => {
    const shouldOpen = window.confirm(
      `リンク先レシピ\n\n卸商品: ${productName}\nレシピ: ${link.recipe_name}\n種別: ${link.link_type}\n\nレシピ詳細を開きますか？`
    );
    if (shouldOpen) router.push(`/recipe/${link.recipe_id}`);
  };
  ...
  // スマホではドラッグ操作の代わりに上下ボタンで並び替える
  const handleMoveProduct = async (id: string, direction: -1 | 1) => {
    const currentList = [...products];
    const currentIndex = currentList.findIndex(p => p.id === id);
    const targetIndex = currentIndex + direction;
    if (currentIndex < 0 || targetIndex < 0 || targetIndex >= currentList.length) return;
  ...
  return (
    <div className="container mx-auto p-3 sm:p-4">
      ...
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <h1 className="text-xl font-bold lg:text-2xl">卸商品マスター管理</h1>
        <div className="grid grid-cols-2 gap-2 lg:flex">
          <Link href="/wholesale/dashboard">
            <Button variant="outline" className="h-11 w-full lg:h-10">ダッシュボードに戻る</Button>
          </Link>
          <Button onClick={() => setShowNewForm(true)} className="flex h-11 items-center gap-2 lg:h-10">
      ...
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center">
        <div className="grid grid-cols-3 overflow-hidden rounded-lg border lg:flex">
          {([['all', `全て (${counts.all})`], ['通常卸', `通常卸 (${counts.通常卸})`], ['OEM', `OEM (${counts.OEM})`]] as const).map(([key, label]) => (
            <button
              ...
              className={`min-h-11 px-2 py-2 text-xs font-medium transition-colors lg:min-h-0 lg:px-3 lg:py-1.5 lg:text-sm ${
                filter === key
        ...
        {selectedIds.size > 0 && (
          <Button
            ...
            disabled={isDeleting}
            className="flex h-11 w-full items-center gap-1.5 lg:h-9 lg:w-auto"
          >
      ...
      {/* 新規登録フォーム */}
      {showNewForm && (
        <div className="mb-4 rounded-lg border bg-gray-50 p-3 sm:p-4">
          ...
          {newForm.product_type === 'OEM' && (
            <div className="mt-2">
              ...
              <select
                ...
                className="h-11 w-full rounded-md border px-3 py-2 text-base lg:h-10 lg:w-64 lg:text-sm"
              >
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 lg:flex">
            <Button onClick={handleAddNew} size="sm" className="h-11 lg:h-9">登録</Button>
            <Button onClick={() => setShowNewForm(false)} size="sm" variant="outline" className="h-11 lg:h-9">キャンセル</Button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3 lg:hidden">
        <div className="flex items-center justify-between rounded-md border bg-white px-3 py-2">
          ...
          <span className="text-xs text-gray-500">{filteredProducts.length}件</span>
        </div>

        {filteredProducts.map((product) => {
          const productIndex = products.findIndex(p => p.id === product.id);
          const recipeLink = recipeLinks.get(product.id);

          return (
            <article
              key={product.id}
              className={`rounded-md border bg-white p-3 shadow-sm ${selectedIds.has(product.id) ? 'border-blue-400 bg-blue-50/40' : 'border-gray-200'}`}
            >
              <div className="flex items-start gap-3">
                <label className="flex min-h-11 items-center" aria-label={`${product.product_name}を選択`}>
                  <Checkbox
                ...
                <div className="min-w-0 flex-1">
                  {editingCell?.id === product.id && editingCell.field === 'product_name' ? (
                    ...
                    <button
                      type="button"
                      onClick={() => startEditing(product.id, 'product_name')}
                      className="min-h-11 w-full break-words text-left text-base font-semibold text-gray-900"
                    >
                      {product.product_name}
                      <span className="ml-2 whitespace-nowrap text-[11px] font-normal text-blue-600">編集</span>
                    </button>
                  )}
                  <div className="text-xs text-gray-500">商品コード: {product.product_code || '-'}</div>
                </div>
                <div className="flex gap-1">
                  ...
                  <Button
                    ...
                    className="h-11 w-11"
                    onClick={() => handleMoveProduct(product.id, 1)}
                    disabled={productIndex < 0 || productIndex >= products.length - 1}
                    aria-label="商品を下へ移動"
                  >
              ...
              {recipeLink && (
                <button
                  type="button"
                  onClick={() => showRecipeLink(product.product_name, recipeLink)}
                  className={`mt-2 inline-flex min-h-10 max-w-full items-center gap-1.5 rounded-md px-3 py-2 text-left text-xs font-medium ${
                    recipeLink.link_type === 'OEM'
                      ? 'bg-purple-100 text-purple-800'
                      : 'bg-emerald-100 text-emerald-800'
                  }`}
                >
                  <Link2 className="h-4 w-4 shrink-0" />
                  <span className="truncate">{recipeLink.recipe_name}</span>
                </button>
              )}
                ...
                <label className="text-xs font-medium text-gray-600">
                  ...
                  {product.product_type === 'OEM' ? (
                    <select
                      ...
                      className="mt-1 h-11 w-full rounded-md border border-gray-200 bg-white px-2 text-base"
                    >
                      <option value="">-- 未設定 --</option>
                      {oemCustomers.filter(c => c.is_active).map(c => (
                        <option key={c.id} value={c.id}>{c.customer_name}</option>
                    ...
                    <span className="mt-1 flex h-11 items-center rounded-md bg-gray-50 px-3 text-sm text-gray-400">対象外</span>
                  )}
                </label>
              </div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                <div>
                  <div className="text-xs font-medium text-gray-600">卸価格</div>
                  {editingCell?.id === product.id && editingCell.field === 'price' ? (
                    ...
                    <button
                      type="button"
                      onClick={() => startEditing(product.id, 'price')}
                      className="mt-1 flex h-11 w-full items-center justify-between rounded-md border px-3 text-base font-semibold"
                    >
                      ¥{product.price.toLocaleString()}
                      <span className="text-[11px] font-normal text-blue-600">編集</span>
                    </button>
                  )}
                </div>
                <div>
                  <div className="text-xs font-medium text-gray-600">利益率</div>
                  {editingCell?.id === product.id && editingCell.field === 'profit_rate' ? (
                    ...
                    <button
                      type="button"
                      onClick={() => startEditing(product.id, 'profit_rate')}
                      className="mt-1 flex h-11 w-full items-center justify-between rounded-md border px-3 text-base font-semibold"
                    >
                      {product.profit_rate}%
                      <span className="text-[11px] font-normal text-blue-600">編集</span>
              ...
              <div className="mt-3 flex min-h-11 items-center justify-between border-t pt-3">
                <div>
                  <div className="text-sm font-medium text-gray-800">販売状態</div>
                  <div className="text-xs text-gray-500">{product.is_active ? '販売中' : '停止中'}</div>
                </div>
                <Switch
                  checked={product.is_active}
                  onCheckedChange={() => handleToggleActive(product.id, product.is_active)}
                  aria-label={`${product.product_name}の販売状態`}
                />
              </div>
            </article>
          );
        ...
        {filteredProducts.length === 0 && (
          <div className="rounded-md border bg-white px-4 py-10 text-center text-sm text-gray-500">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {recipeLinks.has(product.id) && (
                        <span
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            showRecipeLink(product.product_name, recipeLinks.get(product.id)!);
                          }}
                        >
      ...
      {/* フッターサマリー */}
      <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-sm text-gray-500 lg:block">
        表示: {filteredProducts.length}件 / 全{products.length}件
        {selectedIds.size > 0 && <span className="text-blue-600 font-medium lg:ml-3">{selectedIds.size}件選択中</span>}
        <span className="text-emerald-600 font-medium lg:ml-3">
          レシピ紐付: {Array.from(recipeLinks.keys()).filter(id => products.some(p => p.id === id)).length}件
        </span>
        <span className="hidden text-gray-400 lg:ml-3 lg:inline">💡 商品名・卸価格・利益率はクリックで直接編集、Enterで保存  |  ⠿ をドラッグして並び替え</span>
      </div>
    </div>
  );
}
