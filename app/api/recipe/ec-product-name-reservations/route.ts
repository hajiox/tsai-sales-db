...
function toReservationView(row: Record<string, unknown>) {
  ...
  return {
    ...
    newProductName: String(parameters.newProductName || ""),
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "商品名変更予約を取得できません" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (releasable.length > 0) {
      const { data, error } = await supabase.rpc("release_recipe_ec_product_name_batch_jobs", {
      ...
      for (const entry of releasable) {
        ...
        await supabase.from("web_sales_codex_job_events").insert({
          job_id: id,
          event_type: "queued",
          message: "予約したEC商品名変更を一括実行へ移しました",
          progress: 0,
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "商品名変更予約を一括実行できません" }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "商品名変更予約を取り消せません" }, { status: 500 });
  }
}
