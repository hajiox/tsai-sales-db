...
export async function GET(request: Request) {
    ...
    try {
        const { data: recipes, error: recipesError } = await supabase
            .from("recipes")
            .select("id, name, selling_price, total_cost, linked_oem_product_id, category")
            .eq("category", "OEM")
            .order("name");
        ...
        const { data: products, error: productsError } = await supabase
            ...
            .select("id, product_code, product_name, price, profit_rate, is_active, product_type")
            .eq("is_active", true)
            .eq("product_type", "OEM")
            .order("display_order")
            .order("product_code");
        ...
        if (autoMatch) {
            const unlinkedRecipes = (recipes || []).filter((r) => !r.linked_oem_product_id);
            const linkedProductIds = new Set(
                (recipes || []).filter((r) => r.linked_oem_product_id).map((r) => r.linked_oem_product_id)
            );
...
// Helper: 1:1 linking
async function linkRecipeToOemProduct(
    supabase: any,
    recipeId: string,
    ...
    if (productId) {
        const { data: existing } = await supabase
            .from("recipes")
            .select("id")
            .eq("linked_oem_product_id", productId)
            .neq("id", recipeId);
        ...
        if (existing && existing.length > 0) {
            for (const ex of existing) {
                await supabase
                    .from("recipes")
                    .update({ linked_oem_product_id: null })
                    .eq("id", ex.id);
            }
        }
    }
    ...
    await supabase
        .from("recipes")
        .update({ linked_oem_product_id: productId })
        .eq("id", recipeId);
}
...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (body.batch && Array.isArray(body.links)) {
            ...
            for (const link of body.links) {
                await linkRecipeToOemProduct(supabase, link.recipeId, link.productId);
                linked++;
            }

            for (const link of body.links) {
                if (!link.productId) continue;
                await syncPriceToOemProduct(supabase, link.recipeId, link.productId);
            }
        ...
        await linkRecipeToOemProduct(supabase, recipeId, productId);

        if (productId) {
            await syncPriceToOemProduct(supabase, recipeId, productId);
        }
...
export async function PUT() {
    ...
    try {
        const { data: linkedRecipes, error } = await supabase
            .from("recipes")
            .select("id, name, selling_price, total_cost, linked_oem_product_id")
            .not("linked_oem_product_id", "is", null);
