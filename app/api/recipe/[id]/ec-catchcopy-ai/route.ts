...
function toJobView(job: Record<string, unknown>) {
  ...
  return {
    ...
    model: String(parameters.model || EC_CATCHCOPY_AI_MODEL),
    reasoningEffort: String(parameters.reasoningEffort || EC_CATCHCOPY_AI_REASONING_EFFORT),
    rulesVersion: String(parameters.rulesVersion || EC_CATCHCOPY_AI_RULES_VERSION),
...
async function loadGenerationInput(recipeId: string) {
  ...
  const [{ data: recipe, error: recipeError }, { data: syncRows, error: syncError }] = await Promise.all([
    supabase
      .from("recipes")
      .select("id,name,category,series,ec_product_name,ec_product_names_by_site,ec_catchcopies_by_site,product_points,web_description,catchcopy,ingredient_label,filling_quantity,filling_quantity_unit,storage_method,shelf_life,jan_code,product_code")
      .eq("id", recipeId)
      .single(),
    supabase
      .from("recipe_ec_catchcopy_sync_state")
      .select("target,last_catchcopy,updated_at")
      .eq("recipe_id", recipeId),
  ]);
  if (recipeError || !recipe) throw new Error("レシピが見つかりません");
  if (syncError) throw syncError;

  const currentCatchcopies = normalizeEcCatchcopiesBySite(recipe.ec_catchcopies_by_site, recipe.catchcopy);
  const lastVerifiedExternalCatchcopies = Object.fromEntries((syncRows || []).map((row) => [row.target, {
    catchcopy: clip(row.last_catchcopy, 87),
  ...
  const sourceSnapshot = {
    ...
    currentProductName: clip(recipe.ec_product_name, 75),
    currentProductNames: recipe.ec_product_names_by_site || {},
    currentFallbackCatchcopy: clip(recipe.catchcopy, 87),
    currentCatchcopies,
    lastVerifiedExternalCatchcopies,
  ...
  const siteRules = Object.fromEntries(EC_CATCHCOPY_TARGETS.map(({ id, label }) => [id, {
    label,
    platformMaxLength: EC_CATCHCOPY_RULES[id].platformMaxLength,
    preferredMaxLength: EC_CATCHCOPY_RULES[id].generationMaxLength,
    guidance: EC_CATCHCOPY_RULES[id].guidance,
  }]));
...
export async function GET(
  ...
  try {
    ...
    let query = supabase
      ...
      .eq("task_key", "ec_catchcopy_generate")
    ...
    if (requestedJobId && !jobs[0]) {
      return NextResponse.json({ error: "AIキャッチコピー生成タスクが見つかりません" }, { status: 404 });
    }
    ...
    return NextResponse.json({
      error: error instanceof Error ? error.message : "AIキャッチコピー生成タスクを確認できません",
    }, { status: 500 });
  }
}
...
export async function POST(
  ...
  try {
    ...
    const parameters = {
      taskKey: "ec_catchcopy_generate",
      recipeId,
      recipeName: clip(recipe.name, 300),
      sourceSnapshot,
      siteRules,
      model: EC_CATCHCOPY_AI_MODEL,
    ...
    const { data: job, error: insertError } = await supabase
      .from("web_sales_codex_jobs")
      .insert({
        task_key: "ec_catchcopy_generate",
        channel: null,
        trigger_type: "manual",
        period_start: null,
        period_end: null,
        ...
        status: "queued",
        progress: 0,
        current_step: "GPT-5.6 Solのキャッチコピー分析待ち",
      ...
      .select("id,status,progress,current_step,error_message,parameters,result,created_at,started_at,updated_at,completed_at")
      .single();
    if (insertError || !job) throw insertError || new Error("AIキャッチコピー生成タスクを登録できません");
    await supabase.from("web_sales_codex_job_events").insert({
      job_id: job.id,
      event_type: "queued",
      message: "GPT-5.6 Sol専用Skillのキャッチコピー分析を実行待ちに登録しました",
      progress: 0,
      payload: { recipeId, model: EC_CATCHCOPY_AI_MODEL, rulesVersion: EC_CATCHCOPY_AI_RULES_VERSION },
    });
    return NextResponse.json({ ok: true, reused: false, job: toJobView(job) });
  } catch (error) {
    return NextResponse.json({
      error: error instanceof Error ? error.message : "AIキャッチコピー生成タスクを登録できません",
    }, { status: 500 });
  }
}
