// app/api/finance/pl/totals/route.ts
// GET /api/finance/pl/totals?date=YYYY-MM-DD
// - date 未指定: 最新月のトータルを返す
// Node ランタイム（Edge不可）
    ...
    try {
      ...
      if (date) {
        const { rows } = await client.query(
          "SELECT * FROM public.pl_totals_signed_final_v1($1::date)",
          [date]
        );
        ...
        return Response.json(
          {
            ...
            revenues_total: latest?.revenues_total ?? null,
            expenses_total: latest?.expenses_total ?? null,
            net_income_signed: latest?.net_income_signed ?? null,
            diff: latest?.pl_diff ?? null
          },
          { status: 200 }
        );
      }
