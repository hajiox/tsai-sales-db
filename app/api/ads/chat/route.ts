...
### ⚠️ データ解釈の重要な注意点
- conversions_value（コンバージョン値）はGoogle Adsのコンバージョントラッキング設定に完全に依存する
- EC事業者でコンバージョン値の追跡が未設定/不完全な場合、conversions_valueは実際の売上を反映しない（0または極端に低い値になる）
- この場合、ROAS（=conversions_value÷cost）は意味をなさない
- **正しい成果評価の優先順位**: ①CV数（コンバージョン数）②CVR（コンバージョン率）③CPA（獲得単価＝広告費÷CV数）④CTR ⑤CPC
- CPA（Cost Per Acquisition）が最も実用的な費用対効果指標: 1件のCVを獲得するのにいくらかかったか
- 一般的なEC業界のCPA目安: 商品単価の10-30%以下なら良好
- CVR 10%以上は非常に優秀、5%以上は良好、1%未満は要改善
