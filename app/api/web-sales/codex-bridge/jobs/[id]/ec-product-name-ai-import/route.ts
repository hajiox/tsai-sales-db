...
export async function POST(
  ...
  try {
    ...
    if (model !== EC_PRODUCT_NAME_AI_MODEL
    ...
    const result = validateEcProductNameAiResult(body.data);
    ...
    if (job.task_key !== "ec_product_name_generate") {
      return NextResponse.json({ error: "AI商品名生成タスクではありません" }, { status: 409 });
    }
    ...
    const { data: existing, error: existingError } = await supabase
      .from("recipe_ec_product_name_ai_generations")
      .select("id,created_at,model,reasoning_effort,rules_version,suggestions")
      .eq("job_id", id)
      .maybeSingle();
    if (existingError) throw existingError;
    if (existing) {
      const existingResult = validateEcProductNameAiResult(existing.suggestions);
    ...
    return NextResponse.json({
      error: error instanceof Error ? error.message : "AI商品名候補を保存できません",
    }, { status: 500 });
  }
}
