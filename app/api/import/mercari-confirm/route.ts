// /app/api/import/mercari-confirm/route.ts ver.2 (単価スナップショット対応)
  ...
  try {
    ...
    if (newMappings && Array.isArray(newMappings) && newMappings.length > 0) {
      try {
        ...
        const mappingsToInsert = newMappings.map(mapping => ({
          mercari_title: mapping.mercariTitle,
          product_id: mapping.productId
        }));

        const { error: mappingError } = await supabase
          .from('mercari_product_mapping')
          .upsert(mappingsToInsert, { onConflict: 'mercari_title' });
        ...
        console.log(`📚 メルカリ学習データ保存完了: ${learnedCount}件`);
      } catch (mappingError) {
        console.error('メルカリマッピング処理エラー:', mappingError);
        return NextResponse.json(
    ...
    for (const [productId, totalQuantity] of aggregatedSales.entries()) {
      try {
        ...
        if (existingData) {
          // 更新
          console.log(`📝 既存レコード更新: id=${existingData.id}, 旧mercari_count=${existingData.mercari_count}`);
          const { error: updateError } = await supabase
            .from('web_sales_summary')
            .update({ mercari_count: totalQuantity })
          ...
          console.log(`✅ 更新成功: mercari_count=${totalQuantity}`);
        } else {
          // 新規挿入
          ...
          console.log(`✅ 新規挿入成功: mercari_count=${totalQuantity}`);
        }
        successCount++;
      } catch (itemError) {
    ...
    console.log(`メルカリ確定処理完了: 成功${successCount}件, エラー${errorCount}件, 学習${learnedCount}件`);

    return NextResponse.json({
      success: isSuccess,
      message: isSuccess
        ? `メルカリデータの更新が完了しました (成功: ${successCount}件)`
    ...
    console.error('メルカリ確定API エラー:', error);
    return NextResponse.json(
