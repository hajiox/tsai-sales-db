...
export async function POST(request: NextRequest) {
    try {
        ...
        // 未紐付けのMeta広告セットを取得
        const { data: allUnmapped, error: metaError } = await supabase
        ...
        const { data: learnedMaps } = await supabase
            ...
            .select('ad_set_name, series_code')

        const learnedMap = new Map<string, number>()
        learnedMaps?.forEach((m: any) => learnedMap.set(m.ad_set_name, m.series_code))

        let autoApplied = 0
        const metaData: typeof allUnmapped = []

        for (const item of allUnmapped) {
            const learned = learnedMap.get(item.ad_set_name)
            if (learned) {
        ...
        if (metaData.length === 0) {
            return NextResponse.json({ success: true, matched: autoApplied, total: allUnmapped.length, auto_applied: autoApplied, ai_matched: 0, message: '全て学習済みマッピングで紐付けました' })
        }
