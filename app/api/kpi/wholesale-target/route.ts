// /api/kpi/wholesale-target/route.ts — 卸販売月間目標取得API
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('amount')
      .eq('metric', 'target')
      .eq('channel_code', 'WHOLESALE')
      .eq('month', monthKey)
      .maybeSingle();

    if (error) {
      console.error('Wholesale target fetch error:', error);
      return NextResponse.json({ target: 0 });
    }

    return NextResponse.json({ target: data?.amount ?? 0 });
  } catch (err) {
    console.error('Wholesale target API error:', err);
    return NextResponse.json({ target: 0 });
  }
}
