  ...
  return <div className="space-y-6 p-4 lg:p-8">
    ...
    {loading ? <p role="status" className="p-8 text-slate-500">棚卸しデータを取得しています…</p> : report && <>
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border bg-white p-5"><p className="text-sm text-slate-600">{report.hasIncomplete ? "入力済み金額の合計（暫定）" : "棚卸金額の合計"}</p><p className="mt-2 text-3xl font-bold tabular-nums">税別 {yen(report.totalExcluded)}</p><p className="mt-2 text-xl font-bold tabular-nums">税込 {yen(report.totalIncluded)}</p></div>
      ...
      <div className="overflow-x-auto rounded-xl border bg-white"><table className="w-full min-w-[950px] text-sm"><thead className="bg-slate-100 text-left"><tr>{["システム・棚卸し", "棚卸日", "状況", "金額基準", "棚卸金額（税別）", "棚卸金額（税込）", "確認事項", "元画面"].map(label => <th key={label} className="p-4 whitespace-nowrap">{label}</th>)}</tr></thead><tbody>{report.rows.map(row => <tr key={row.key} className="border-t align-top"><td className="p-4"><p className="text-xs text-slate-500">{row.system}</p><p className="mt-1 font-semibold">{row.label}</p></td><td className="p-4 whitespace-nowrap">{row.date ?? "—"}</td><td className="p-4 whitespace-nowrap"><span className={`rounded px-2 py-1 text-xs ${row.status === "completed" ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}`}>{statusLabel(row.status)}</span></td><td className="p-4">{row.basis}</td><td className="p-4 text-right font-semibold tabular-nums whitespace-nowrap">{yen(row.amountExcluded)}</td><td className="p-4 text-right font-semibold tabular-nums whitespace-nowrap">{yen(row.amountIncluded)}</td><td className="max-w-60 p-4 text-amber-800">{row.warning || (row.status === "missing" ? "この年度の棚卸しは未作成です" : "—")}</td><td className="p-4"><Link href={row.href} target="_blank" rel="noopener noreferrer" aria-label={`${row.label}の元画面`} className="inline-flex items-center gap-1 whitespace-nowrap text-blue-700">開く<ExternalLink size={14}/></Link></td></tr>)}</tbody><tfoot className="border-t bg-slate-50 font-bold"><tr><td colSpan={4} className="p-4">{report.hasIncomplete ? "入力済み金額の合計（暫定）" : "合計"}</td><td className="p-4 text-right whitespace-nowrap">{yen(report.totalExcluded)}</td><td className="p-4 text-right whitespace-nowrap">{yen(report.totalIncluded)}</td><td colSpan={2}/></tr></tfoot></table></div>
      <div className="space-y-2 text-xs text-slate-500"><p>税別・税込を分け、円未満を切り捨てて集計しています。食のブランド館は元Excelのシート合計で税換算し、その他は明細ごとに換算します。未入力分は金額に含みません。Excelには一覧と各棚卸しの明細を出力します。</p><p>取得日時：{new Date(report.fetchedAt).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })}　元画面で保存した変更は「最新情報に更新」で反映されます。</p></div>
    </>}
  </div>;
}
