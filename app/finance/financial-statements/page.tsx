...
function FinancialStatementsInner() {
  ...
  // 人件費（小計はコード or 内訳合算）
  const personnelRows = PERSONNEL.map((r) => ({ label: r.label, value: amt(r.code) }));
  const personnelSubtotal = amt(PERSONNEL_SUBTOTAL.code) || sum(PERSONNEL.map((r) => r.code));

  // その他販管費
  const sgaDetailRows = SGA_DETAILS.map((r) => ({ label: r.label, value: amt(r.code) }));
  const sgaSubtotal =
  ...
  // 営業外収益・費用
  const nonOpIncRows = NONOP_INCOME_DETAILS.map((r) => ({ label: r.label, value: amt(r.code) }));
  const nonOpIncSub = amt(NONOP_INCOME_SUBTOTAL.code) || sum(NONOP_INCOME_DETAILS.map((r) => r.code));
  const nonOpExpRows = NONOP_EXPENSE_DETAILS.map((r) => ({ label: r.label, value: amt(r.code) }));
  const nonOpExpSub = amt(NONOP_EXPENSE_SUBTOTAL.code) || sum(NONOP_EXPENSE_DETAILS.map((r) => r.code));
  ...
  const netIncomeAbs = amt(NET_INCOME.code) || Math.abs(netIncomeYtd ?? 0);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <SectionCard title="損益計算書（当期累計）">
              ...
              <div className="divide-y mb-2">
                {salesRows.map((r) => (
                  <Line key={r.label} name={r.label} value={r.value} />
                ))}
              ...
              <div className="divide-y">
                ...
                {/* 当期純利益はコード優先／無ければRPCのYTD */}
                <div className="border-t mt-3 pt-2 flex items-baseline justify-between">
                  <span className="font-semibold">{NET_INCOME.label}</span>
                  <span className="font-semibold">{yen(netIncomeAbs)}</span>
                </div>
              </div>

              <div className="text-xs text-slate-500 mt-3">
                ※ 勘定コードはPDFの様式に合わせています。DBにコードが無い行は0で表示し、
                一部の小計／合計は内訳合算で補完しています。
              </div>
            </SectionCard>
