...
const SITE_COLORS: Record<EcCatchcopyTarget, string> = {
  rakuten: "bg-red-500 hover:bg-red-600",
  yahoo: "bg-violet-600 hover:bg-violet-700",
};

function statusLabel(status: EcCatchcopyJobView["status"]) {
  return ({
...
export default function EcCatchcopySyncControls({
  recipeId,
  recipeName,
  fallbackCatchcopy,
  ecCatchcopiesBySite,
  ...
  const [job, setJob] = useState<EcCatchcopyJobView | null>(null);
  const [history, setHistory] = useState<EcCatchcopyHistoryEntry[]>([]);
  ...
  const refresh = useCallback(async () => {
    const response = await fetch(`/api/recipe/${recipeId}/ec-catchcopy-jobs`, { cache: "no-store" });
  ...
  const refreshReservations = useCallback(async () => {
    const response = await fetch("/api/recipe/ec-catchcopy-reservations", { cache: "no-store" });
    if (!response.ok) return;
    const payload = await response.json();
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      const response = await fetch(`/api/recipe/${recipeId}/ec-catchcopy-jobs?jobId=${activeJobId}`, { cache: "no-store" });
      if (!response.ok) return;
      const payload = await response.json();
  ...
  useEffect(() => {
    ...
    if (job.status === "completed") toast.success(job.summary || "EC用キャッチコピーの変更が完了しました");
    else if (job.status === "waiting_for_user" || job.status === "needs_review") toast.warning(job.summary || "キャッチコピー変更結果の確認が必要です");
    else toast.error(job.errorMessage || job.summary || "EC用キャッチコピーの変更に失敗しました");
    void refresh();
  }, [job, refresh]);
  ...
  const canRetry = Boolean(job
    ...
    && ecCatchcopyMapsEqual(targetCatchcopies, job.catchcopies, summaryCatchcopy));
  ...
  async function enqueue(targets: EcCatchcopyTarget[]) {
    if (disabled || targets.some((target) => !targetCatchcopies[target])) return;
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`/api/recipe/${recipeId}/ec-catchcopy-jobs`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          targets,
          dispatchMode,
          expectedCatchcopy: summaryCatchcopy,
          expectedCatchcopies: targetCatchcopies,
          expectedRecipeSnapshot,
        }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "キャッチコピー変更を登録できません");
      if (payload.reserved) {
        await refreshReservations();
        setShowReservations(true);
        toast.success("キャッチコピー変更を一括実行予約へ追加しました");
      } else {
        setJob(payload.job);
        notified.current = null;
        toast.success("事務所PCのCodexへキャッチコピー変更を登録しました");
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "キャッチコピー変更を登録できません");
    } finally { setSubmitting(false); }
  }

  async function retryUnfinished() {
    if (!job || !canRetry || disabled) return;
    if (!window.confirm(`未完了の${getEcCatchcopyTargetLabel(unfinishedTargets)}だけを再実行しますか？`)) return;
    setSubmitting(true);
    try {
      const response = await fetch(`/api/recipe/${recipeId}/ec-catchcopy-jobs`, {
  ...
  async function executeReservations() {
    ...
    if (!window.confirm(`予約済み${reservations.length}件を順番に実行しますか？`)) return;
    setQueueAction(true);
    try {
      const response = await fetch("/api/recipe/ec-catchcopy-reservations", {
  ...
  async function cancelReservation(reservation: ReservationView) {
    if (queueAction || !window.confirm(`「${reservation.recipeName}」のキャッチコピー変更予約を取り消しますか？`)) return;
    setQueueAction(true);
    try {
      const response = await fetch("/api/recipe/ec-catchcopy-reservations", {
  ...
  return (
    <div className="mt-3 border-t border-blue-100 pt-3">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <button type="button" disabled={disabled || EC_CATCHCOPY_TARGETS.some(({ id }) => !targetCatchcopies[id])} onClick={() => void enqueue(EC_CATCHCOPY_TARGETS.map((target) => target.id))}
      ...
      {(job || reservations.length > 0 || history.length > 0) && <div className="mt-3 space-y-2">
        ...
        {showReservations && <div className="space-y-2 text-xs">
          {reservations.length === 0 ? <p className="rounded bg-gray-50 p-2 text-gray-500">予約はありません。</p> : <>
            {reservations.map((reservation) => <div key={reservation.id} className="flex items-center justify-between gap-2 rounded border bg-white p-2">
              <div className="min-w-0"><p className="truncate font-bold">{reservation.recipeName}</p><p className="truncate text-gray-500">{reservation.newCatchcopy} / {getEcCatchcopyTargetLabel(reservation.targets)}</p></div>
        ...
        <button type="button" onClick={() => setShowHistory((value) => !value)} className="flex w-full items-center justify-between border-t pt-2 text-xs font-bold text-gray-700">
          <span><History className="mr-1 inline h-4 w-4" />ECキャッチコピー変更履歴 {history.length}件</span>{showHistory ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
        </button>
        {showHistory && <div className="max-h-64 space-y-2 overflow-y-auto text-[11px]">{history.map((entry) => <div key={entry.id} className="rounded border bg-white p-2">
          <div className="flex justify-between gap-2"><span className="font-bold">{entry.catchcopies.rakuten || entry.catchcopies.yahoo}</span><span className="shrink-0 text-gray-400"><Clock3 className="mr-1 inline h-3 w-3" />{new Date(entry.createdAt).toLocaleString("ja-JP")}</span></div>
          {entry.sites.map((site) => <p key={site.site} className="mt-1 text-gray-600">{site.site}: {site.previousCatchcopy || "未確認"} → {site.finalCatchcopy || site.newCatchcopy}（{siteStatusLabel(site.status)}）</p>)}
        </div>)}</div>}
      </div>}
    </div>
  );
