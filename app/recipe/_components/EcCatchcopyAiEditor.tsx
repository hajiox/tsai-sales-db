...
type Props = {
  recipeId: string;
  fallbackCatchcopy: string | null | undefined;
  catchcopiesBySite: EcCatchcopiesBySite | null | undefined;
  onChange: (catchcopies: EcCatchcopiesBySite, fallbackCatchcopy?: string) => void;
};

const SITE_ACCENTS: Record<EcCatchcopyTarget, string> = {
  rakuten: "border-l-red-500",
  yahoo: "border-l-violet-500",
};

export default function EcCatchcopyAiEditor({ recipeId, fallbackCatchcopy, catchcopiesBySite, onChange }: Props) {
  ...
  const current = useMemo(
    () => normalizeEcCatchcopiesBySite(catchcopiesBySite, fallbackCatchcopy),
    [catchcopiesBySite, fallbackCatchcopy],
  );

  function updateSite(target: EcCatchcopyTarget, value: string) {
    onChange({ ...current, [target]: normalizeEcCatchcopyForTarget(target, value) });
  }
  ...
  const finishGeneration = useCallback((job: GenerationJob) => {
    ...
    setOpen(true);
    toast.success("GPT-5.6 Solのキャッチコピー候補を作成しました");
  }, []);
  ...
  function applyAll() {
    ...
    const catchcopies = Object.fromEntries(
      EC_CATCHCOPY_TARGETS.map(({ id }) => [id, generation.suggestions[id].catchcopy]),
    ) as EcCatchcopiesBySite;
    onChange(catchcopies, generation.suggestions.rakuten.catchcopy);
    setOpen(false);
    toast.success("各ECのキャッチコピー候補を採用しました。内容を確認してレシピを保存してください");
  }

  function applyOne(target: EcCatchcopyTarget) {
    if (!generation) return;
    updateSite(target, generation.suggestions[target].catchcopy);
    toast.success(`${EC_CATCHCOPY_TARGETS.find(({ id }) => id === target)?.label}の候補を採用しました`);
  }
  ...
  return (
    <div className="mt-3 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-gray-200 pt-3">
        <div>
          <p className="text-sm font-bold text-gray-800">EC別のキャッチコピー</p>
          <p className="text-[11px] text-gray-500">楽天とYahooの専用欄へ、サイト別に保存します。</p>
        </div>
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          >
            ...
            {generating ? `GPT-5.6 Solで分析中 ${jobProgress}%` : "AIで各EC向けキャッチコピーを作成"}
          </button>
        </div>
      </div>
      ...
      <div className="divide-y rounded-md border border-gray-200 bg-white">
        {EC_CATCHCOPY_TARGETS.map(({ id, label }) => {
          const rule = EC_CATCHCOPY_RULES[id];
          ...
          return (
            <div key={id} className={`border-l-4 px-3 py-3 ${SITE_ACCENTS[id]}`}>
              <div className="mb-1 flex items-center justify-between gap-2">
                <label className="text-xs font-bold text-gray-800" htmlFor={`ec-catchcopy-${id}`}>{label}</label>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] max-w-5xl overflow-hidden p-0">
          <DialogHeader className="border-b px-5 pb-4 pt-5 pr-12">
            <DialogTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-blue-600" />ECキャッチコピー AI分析</DialogTitle>
              ...
              {EC_CATCHCOPY_TARGETS.map(({ id, label }) => {
                ...
                return <div key={id} className="grid gap-3 py-4 md:grid-cols-[100px_minmax(0,1fr)_130px]">
                  ...
                  <div className="min-w-0">
                    <p className="break-words text-sm font-bold leading-relaxed text-gray-950">{item.catchcopy}</p>
                    <p className="mt-1 text-[11px] text-gray-500">{item.catchcopy.length}/{rule.platformMaxLength}文字 ・ {item.rationale}</p>
