// app/recipe/wholesale-link/page.tsx
// 卸販売商品 → レシピ 紐付け管理（自社向け）
"use client";
...
export default function WholesaleLinkPage() {
    ...
    const handleAutoMatch = async () => {
        setMatching(true);
        try {
            const res = await fetch("/api/recipe/sync-wholesale?autoMatch=true");
    ...
    const handleSyncAll = async () => {
        setSyncing(true);
        try {
            const res = await fetch("/api/recipe/sync-wholesale", { method: "PUT" });
    ...
    // Derived
    const linkedRecipes = recipes.filter(r => r.linked_wholesale_product_id);
    const linkedProductIds = new Set(linkedRecipes.map(r => r.linked_wholesale_product_id!));
    const getRecipeForProduct = (productId: string) => recipes.find(r => r.linked_wholesale_product_id === productId);
        ...
        <div className="max-w-[1400px] mx-auto">
            ...
            <div className="mb-6 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">
                        卸販売管理の全商品にレシピを紐付けて、卸価格・利益率を自動同期（7掛ベース）
                    </p>
                </div>
                <div className="flex gap-2">
                    {step === "overview" && (
                        <>
                            ...
                            <Button onClick={handleAutoMatch} disabled={matching || unlinkedProducts.length === 0} className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700">
            ...
            <div className="mb-6">
                ...
                <div className="w-full bg-gray-200 rounded-full h-3">
                    <div className={`h-3 rounded-full transition-all duration-500 ${isComplete ? "bg-green-500" : "bg-green-400"}`} style={{ width: `${progressPercent}%` }} />
                </div>
                {isComplete && (
                    <div className="mt-2 flex items-center gap-2 text-green-600 font-medium">
                        <CheckCircle2 className="w-5 h-5" /> 全商品の紐付けが完了しています！
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
                <Card><CardContent className="pt-4 pb-3"><div className="text-sm text-gray-500">卸販売商品</div><div className="text-2xl font-bold">{totalProductCount}</div></CardContent></Card>
                <Card><CardContent className="pt-4 pb-3"><div className="text-sm text-gray-500">レシピ（自社）</div><div className="text-2xl font-bold">{recipes.length}</div></CardContent></Card>
                <Card className="border-green-200 bg-green-50"><CardContent className="pt-4 pb-3"><div className="text-sm text-green-700">紐付け済み</div><div className="text-2xl font-bold text-green-700">{linkedCount}</div></CardContent></Card>
                <Card className={`${unlinkedProducts.length === 0 ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"}`}><CardContent className="pt-4 pb-3"><div className={`text-sm ${unlinkedProducts.length === 0 ? "text-green-700" : "text-amber-700"}`}>未紐付け</div><div className={`text-2xl font-bold ${unlinkedProducts.length === 0 ? "text-green-700" : "text-amber-700"}`}>{unlinkedProducts.length}</div></CardContent></Card>
            </div>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {suggestions.map(s => {
                                    ...
                                    return (
                                        <TableRow key={s.recipeId} className={s.accepted ? "bg-green-50/50" : s.confidence === "none" ? "bg-gray-50/50" : ""}>
                        ...
                        <Card className="mb-6 border-amber-200">
                            <CardHeader className="pb-3">
                                <CardTitle className="text-lg flex items-center gap-2 text-amber-700">
                                    <Circle className="w-5 h-5" /> 未紐付け 卸販売商品（{unlinkedProducts.length}件）
                                </CardTitle>
                    ...
                    <Card className="mb-6 border-green-200">
                        <CardHeader className="pb-3">
                            <CardTitle className="text-lg flex items-center gap-2 text-green-700">
                                <CheckCircle2 className="w-5 h-5" /> 紐付け済み 卸販売商品（{linkedCount}件）
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                                        ...
                                        {linkedProducts.map(product => {
                                            ...
                                            return (
                                                <TableRow key={product.id}>
                                                    <TableCell><CheckCircle2 className="w-4 h-4 text-green-500" /></TableCell>
                                                    <TableCell className="font-medium">{product.name}</TableCell>
                                                    <TableCell className="text-right">{formatCurrency(product.price)}</TableCell>
                                                    <TableCell className="text-right">{product.profit_rate != null ? `${product.profit_rate}%` : "-"}</TableCell>
                                                    <TableCell className="text-center"><Link2 className="w-4 h-4 mx-auto text-green-500" /></TableCell>
                                                    <TableCell className="font-medium text-green-700">{recipe?.name || "（レシピ不明）"}</TableCell>
