...
## 2026-09-15 取込元の訂正
- ユーザー指定のDownloads版へ2026年度を差し替え。5シート・471非空セル・118計算式。合計、ブランド館フロア資材在庫、道の駅食材在庫、道の駅資材在庫、共通資材のみ。
- 旧8シート版はrevision 1の履歴へ全保存し、現在値と取込原本を新ファイルへ更新（revision 2）。通常APIの原本保護は維持し、専用訂正処理内のトランザクションで監査・差し替え・保護復帰を完結。
- 新原本の参照切れは合計B3/B4/B7/B11の4セル。原本どおり維持。全セルDB照合、118式再計算比較、旧データ保管、監査トリガー復帰、本番5タブ・PC/スマホ確認に合格。画面コード変更なし、DB反映で完了。
- 新版検証: node scripts/test-food-store-inventory.cjs <新版xlsx> 5。既存本番の読取検証: node scripts/check-food-store-inventory-ui.cjs https://v0-tsa-19.vercel.app 5。
