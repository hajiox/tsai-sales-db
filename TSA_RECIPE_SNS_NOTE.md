...
- 1.9.83は対話CLI専用引数を`codex exec`がブラウザー開始前に拒否した。1.9.84で有効な`approval_policy="never"`へ直すと、非対話Codexが通常CUA呼出しを本人未回答として拒否した。さらに`--approve-for-me`へ戻し、固定画像を開始入力にも添付した独立検証では、Xの公式file chooser取得後の`setFiles`だけが`permission request was dismissed before a decision was made`で停止し、画像プレビュー・本文入力・投稿はいずれもなかった。
- Bridge 1.9.85では通常CUAを動かせる自動レビューへ戻し、保護された画像送信が本人確認を完了できない場合は`waiting_for_user`で閉じる。現行の非対話CUAには通常MCPの自動レビューとローカル画像送信の本人判断を同時に成立させるモードがない。完全無人化にはX APIとMeta Graph APIの認証設定が必要だが、ローカル・Vercelとも該当する認証環境変数は未登録だった。
...
- それでも4媒体の画像数が不足した場合は、固定済み入力を変えず新規の隔離セッションで全体を1回だけ自動再試行する。成功画像数の完全一致、生成順、許可保存先の検証は維持する。
- TSA画面は保存済みの実モデル名とBridgeの失敗理由を表示し、失敗した画像欄で進行中スピナーを出さない。

## 2026-09-02

### 開発者口調の敬体化

- 開発者口調の一人称を「私」へ固定し、「です・ます」調を基本にした。商品の設計意図や大切にしている点は具体的に説明するが、読み手へ押し付けず、選択を委ねる穏やかな発信にする。
- 専用SNS Skillsから「俺」「歯切れよく」「譲れない」の指示を除き、命令、説教、挑発、優越表現、強い断定、他社批判を禁止した。「絶対」「これが正解」「〜すべき」「〜してほしい」なども避ける。
- Bridge取込時に、新しい開発者口調が「俺・僕」などの一人称や代表的な高圧表現を含む場合は保存を拒否する。過去の不変履歴は引き続き閲覧できる。
- Bridge 1.9.56、SNSルール`2026-09-02.3`へ更新した。
- コミット`c02e19c6`を本番エイリアスへ反映し、Bridge 1.9.56の通常・ログイン前ワーカーのheartbeatを確認した。
- 誤った旧口調で完成していた履歴`7d480de9-7158-4d37-8e73-61373ed1bb33`は変更せず、4媒体の既存画像をそのまま再利用した補正履歴`4132e64f-bb58-4cd0-a642-ebad2370a1dd`を追加した。補正履歴は一人称「私」、高圧表現なし、商品LP各1件、外部投稿0件を確認した。
...
- 初回1.9.55再実行で、4画像artifactの作成後に本番APIの旧検証が空の`file_path`を拒否した。ローカル絶対パスは認証済みartifactの根拠にしないため、APIは生成元種別と4媒体artifactを検証し、AI由来のローカルパスを必須としない契約へ統一した。
...
- `オフィシャル`は落ち着いたマーケッター視点、`スタッフ`は世間話と控えめなユーモアを含む明るく長めの文章、`開発者`は一人称「私」の丁寧な口調で商品ポイントとこだわりを掘り下げる。入力にない実体験、試作回数、客の反応、比較優位を創作しない境界を専用Skillへ追加した。
...
## 2026-09-07 SNS投稿先アカウント選択
- XはAizu_Brand_Kan / karasugike1 / hajiox、Instagram・IGストーリー・Threadsはaizubrandhall / satou.masahikoを媒体ごとに選択。既定値と既存予約は維持。
- 固定スナップショット・投稿確認・履歴・Bridge入力・成功結果照合へ選択値を引継ぐ。Bridge 1.9.70は旧ルールと新ルールを受理し、新ルールのみ複数候補を許可。ログイン済みの指定アカウントだけ切替可。
- 全9組合せ、不正入力、異なる成功アカウント拒否、旧予約互換、SNS契約、security/RLS、Next buildを確認。全体tscは既存エラー（変更ファイル該当なし）。実投稿テストは行っていない。

## 2026-09-07 Bridge内でのブラウザー確認継続（1.9.71）
- SNS専用のMCP確認中継を追加。公式CUAの確認フォームを事務所PCに表示し、本人の回答だけを同じ保留中呼出しへ返す。確認待ちでもジョブとheartbeatを保持し、別Codexタスクへの移動を不要にした。通常の固定内容の投稿は公開・結果保存まで続行。
- 一律の非対話停止指示と非表示file input優先を修正。chooserタイムアウトを権限拒否に変換しない。拒否・中止・未回答・未対応フォームは停止し、同意の自動生成は行わない。5分の回答期限と親終了時の閉鎖を設定。
- 確認応答の型検証、未回答中の非送信、回答後のMCP継続、取消、SNS契約、全9アカウント組合せ、security/RLS、本番ビルドを確認。非対話実機でBrowser確認のBridge画面表示を確認。検証用の実投稿は行わない。

## 2026-09-07 終了結果の自動照合（Bridge 1.9.72）
- PCのモニターが過去の操作待ち表示を再利用し続ける問題を修正。待機時のheartbeatで同じworker・同じジョブのサーバー終了結果を読み、ローカル表示へ反映。本文・画像・実行は再処理しない。
- 別ジョブ・処理中・古い応答・不正日時を反映しない。APIは認証とworker ID照合を維持し、終了結果の最小項目だけ返す。旧クライアントは照会なしで互換。状態遷移・worker照合・認証・旧クライアント・監視テストとsecurity/RLSを確認。

## 2026-09-09 投稿完了までの実行指示（Bridge 1.9.76）
- Skillの非対話実行に関する古い停止指示を削除し、投稿前確認はAI自身の確定値照合と明記。Bridgeの媒体別プロンプトも下書き・プレビューでは終えず、画像添付から公開検証まで続行する契約に統一。
- 実際の認証・ブラウザー安全確認・拒否は維持。SNS契約、全9アカウント、通常操作の確認なし通過、確認中継の拒否保持、security/RLSを検証。検証用の実投稿は実施しない。

## 2026-09-09 媒体別一時タブと投稿承認伝達（Bridge 1.9.77）
- 媒体別セッションが既存SNSタブを取得して競合する問題を修正。各媒体はログイン状態を共有する専用一時タブだけを使い、IGストーリーはMeta Business Suiteから開始する。
- TSAの確定操作が固定画像のアップロードと最終公開を明示承認していることをセッションへ伝達。対象外操作、認証・権限・安全確認の拒否停止は維持。
- SNS契約、全9アカウント、確認中継、security/RLSを確認。本番再実行で公開結果を検証する。

## 2026-09-11 投稿開始確認のデバッグ操作対応
- 全SNS投稿の開始確認をブラウザー標準の`window.confirm`からページ内のAlertDialogへ変更。標準確認がCDP接続を占有してCodexのデバッグ操作を止める問題を解消し、投稿先を表示したまま「投稿を確定」を操作できるようにした。
...
## 2026-09-12 X送信確認とIGストーリー可読性（Bridge 1.9.87）
- 実投稿でXだけ画像添付後の投稿ボタン押下が送信されず、Instagram・IGストーリー・Threadsだけ成功した事例を確認。Xは画像処理完了と投稿ボタン有効化を待ち、押下後45秒の成功・入力欄・status/alert・プロフィール確認を必須化した。完全に未送信と証明できた場合だけ2回目を許可し、不明時の重複投稿は防ぐ。
- Meta Business Suiteの初期文字サイズが小さいままIGストーリーを成功扱いにしたため、全文が読みやすく、原則2〜4行かつ画像幅の半分以上になることを公開前の合格条件へ追加。小さい・読めない状態を成功結果として受理しない。
- 複数媒体の一部だけ失敗した履歴表示を「確認待ち」から「一部失敗」へ明確化した。

## 2026-09-12 X複数行本文の入力修正（Bridge 1.9.88）
- XのDraft.js編集欄へ複数行本文を`fill`した実行で改行が消え、商品URLと次行のハッシュタグが結合したことを実機で再現。本文を行単位の`type_text`と改行ごとの`press_key Enter`で入力する手順へ変更した。
- 画像添付前と送信直前にDraft.jsの行ブロックをLFで再構成し、固定本文との完全一致を必須化。不一致時の入力やり直しは1回だけとし、再不一致なら未送信で停止する。

## 2026-09-12 X送信待機順序の修正（Bridge 1.9.89）
- 1.9.88の実投稿で本文完全一致と画像添付は通過したが、投稿ボタン押下直後に待機せずプロフィールへ移動し、beforeunloadで下書きを破棄したため公開不能となった。
- 各押下直後の`wait_for` 45秒を必須化。同じ下書きが残る間は画面遷移せず、新しいsnapshotで未送信条件を確認して同じ編集画面で2回目を実行し、その待機後にだけプロフィール確認する。

## 2026-09-13 Instagram投稿先変更（Bridge 1.9.90）
- Instagram通常投稿の選択肢 satou.masahiko を aizubrand_ec に変更。画面/API/Bridgeの許可リストと投稿Skillを一致させた。IGストーリー・Threadsの候補は従来どおり。過去の投稿履歴や固定済み予約は書き換えない。
- アカウント9組合せ、誤アカウント拒否、投稿契約、secret scan、RLS検査に合格。
- 本番ビルド188ページ生成に合格。
